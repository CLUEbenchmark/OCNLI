...
def evaluate_full(classifier, eval_set, batch_size, name='eval',ofile=False):
    """
    Get comma-separated CSV of predictions.
    Output file has two columns: pairID, prediction
    """
    ...
    logits,predictions,cost = classifier(eval_set)
    outputs = []
    correct = 0.
    total = 0.

    for i in range(len(eval_set)):
        orig_info = eval_set[i]
        predicted_label = predictions[i]

        ## predicted label 
        prediction = INVERSE_MAP[predicted_label]
        if prediction == orig_info["gold_label"]: correct += 1.

        total += 1.
        outputs.append("%s\t%s\t%s\t%s\t%s" % (orig_info["sentence1"],
                                        orig_info["sentence2"],
                                        orig_info["gold_label"],
                                        prediction,
                                        ' '.join(["%s:%f" % (INVERSE_MAP[k],l) for k,l in enumerate(logits[i])])))
    ...
    ## print to file?
    if ofile:
        with open(os.path.join(ofile,name + '_predictions.csv'), 'w') as out_file: 
            for item in outputs:
                out_file.write(item)
                out_file.write("\n")
    return accuracy,int(total)    
